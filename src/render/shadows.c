...
/*   Updated: 2025/03/31 17:37:05 by jgraf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float	calculate_shadow_intensity(t_scene_data *data, t_vector intersect, t_vector light_pos, t_ray shadow_ray)
{
	t_vector	jitter_pos;
	t_vector	jitter_dir;
...
float	shadow_caster(t_scene_data *data, t_vector intersect, t_vector normal, t_light *light)
{
	...
	if (vec_dot(normal, light_dir) < 0)
		return (0);
	return (1.0f - (calculate_shadow_intensity(data, intersect, light_pos, shadow_ray) / data->samples));
}

static float	check_ao_intersections(t_scene_data *data, t_ray ao_ray, float max_distance)
{
	t_asset_node	*node;
	float			occlusion;
	double			t;
	...
	while (node)
	{
		...
		{
			if (plane_hit((t_plane *)node->asset_struct, ao_ray, &t) && t > 0.001f && t < max_distance)
				occlusion += 1.0f;
		}
		node = node->next;
	}
