	...
	double *closest_t, t_asset_type *closest_type)
{
	...
	*closest_type = NO_TYPE;
	...
	while (node)
	{
		if (((node->type == AST_PLANE && plane_hit((t_plane *)node->asset_struct, ray, &t))
			|| (node->type == AST_SPHERE && sphere_hit((t_sphere *)node->asset_struct, ray, &t))
			|| (node->type == AST_CYLINDER && cylinder_hit((t_cylinder *)node->asset_struct, ray, &t)))
			&& (t < *closest_t))
		{
			*closest_t = t;
			closest_node = node;
...
static uint32_t	calculate_lighting(t_render *render, int depth)
{
	...
	light_pos = vec_new(lighting.light->pos_x, lighting.light->pos_y, lighting.light->pos_z);
	lighting.light_dir = vec_normalize(vec_sub(light_pos, lighting.intersect));
	lighting.shadow_intensity = shadow_caster(render->data, lighting.intersect, lighting.normal, lighting.light);
	lighting.ao = ambient_occlusion(render->data, lighting.intersect, lighting.normal);
	...
	base_color = merge_color(base_color, add_reflection(render, lighting, depth), render->closest_node->reflect);
	return (base_color);
}
