...
/*   Updated: 2025/03/31 17:34:34 by jgraf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t	add_reflection(t_render *render, t_lighting lighting, int depth)
{
	...
	{
		reflect_dir = vec_sub(render->ray.direction, vec_scale(lighting.normal, 2 * vec_dot(render->ray.direction, lighting.normal)));
		reflected_ray.origin = vec_add(lighting.intersect, vec_scale(reflect_dir, 0.001f));
