...
/*   Updated: 2025/03/31 17:33:28 by jgraf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vector	calculate_pixel_position(t_camera *cam, float px, float py)
{
	...
	x_offset = (2 * ((px + 0.5) / WIDTH) - 1) * ((float)WIDTH / (float)HEIGHT) * fov_adjustment;
	...
	return (vec_normalize(vec_add(direction, vec_add(vec_scale(right, x_offset), vec_scale(vec_cross(right, direction), y_offset)))));
}
