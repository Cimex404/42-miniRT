...
/*   Updated: 2025/03/31 17:30:54 by jgraf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	sideways(mlx_key_data_t keydata, t_scene_data *data, float rot_spd)
{
	...
	{
		data->cam->vec_z = data->cam->vec_x * sin(rot_spd) + data->cam->vec_z * cos(rot_spd);
		data->cam->vec_x = data->cam->vec_x * cos(rot_spd) - data->cam->vec_z * sin(rot_spd);
	}
...
bool	rotation_keys(mlx_key_data_t keydata, t_scene_data *data)
{
	...
	{
		len = sqrt(data->cam->vec_x * data->cam->vec_x + data->cam->vec_y * data->cam->vec_y + data->cam->vec_z * data->cam->vec_z);
		data->cam->vec_x /= len;
		data->cam->vec_y /= len;
		data->cam->vec_z /= len;
		return (true);
