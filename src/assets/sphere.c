...
/*   Updated: 2025/03/31 17:30:00 by jgraf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	calculate_discriminant(t_sphere *sphere, t_ray ray, t_quad *quad)
{
	...
	oc = vec_sub(ray.origin, vec_new(sphere->pos_x, sphere->pos_y, sphere->pos_z));
	b = 2.0 * vec_dot(oc, ray.direction);
	discriminant = b * b - 4 * vec_dot(ray.direction, ray.direction) * (vec_dot(oc, oc) - pow(sphere->diameter / 2.0f, 2));
	if (discriminant < 0)
		return (false);
	quad->t0 = (-b - sqrt(discriminant)) / (2.0 * vec_dot(ray.direction, ray.direction));
	quad->t1 = (-b + sqrt(discriminant)) / (2.0 * vec_dot(ray.direction, ray.direction));
	return (true);
}
